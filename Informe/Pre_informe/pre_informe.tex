...
Se definió utilizar una estructura con dos ruedas y un punto de apoyo. El frente del robot se encuentra en el lado opuesto al del punto de apoyo, con una estructura para soporte de los LEDs del sensor de línea colocada en esta zona. La estructura diseñada se muestra en la figura \ref{estructura robot}.
\begin{figure}[h] %$.$ Faltan los encoders
\label{estructura robot}
\centering
...
\end{equation}
 ...
 \subsection{Modelado de los motores2 }
 \label{modeloMotores}
...
Para alcanzar los objetivos planteados se decidió implementar tres controladores PID en simultáneo: el controlador para el sistema total, que se encargará de que el robot siga la pista, y dos controladores para regularizar el comportamiento de los motores. Estos últimos se utilizan para que los motores se muevan a la velocidad requerida y para compensar asimetrías en la respuesta de los dos motores.

A continuación se detallan los ensayos realizados para la identificación de los sistemas a controlar.
%$.$ Todos estos controladores requieren algún tipo de conocimiento sobre la planta. A fin de obtener información del comportamiento de cada sistema se realizaron ensayos de respuesta al escalón.
...
\begin{figure}
\centering
\includegraphics[width=15cm]{./imagenes/prueba_linealidad.eps}
...
En función de las consideraciones anteriores, se diseño un ensayo que permitiese obtener la respuesta al escalón de ambos motores. Dado que nos interesa conocer el comportamiento con carga y no en vacío, se buscó que el ensayo se realizara con el robot completo funcionando en condiciones normales. Para ello se programó para que el robot anduviera en línea recta, primero a una velocidad baja y luego a uno mayor dada por un cambio repentino del duty cycle de ambos motores. Los valores obtenidos para las variables de interés se almacenaron en la EEPROM y luego fueron transmitidos a la computadora por puerto serie. La curva obtenida se muestra en la figura \ref{EscalonMotores}.
\begin{figure}[h]
\centering
...
A partir de estos datos se estimó un sistema LTI contínuo que se ajustara a cada curva utilizando la herramienta de Matlab System Identification. Las funciones de transferencias obtenidas para cada motor fueron:
\begin{equation}
...
La cinemática del robot está gobernada por las ecuaciones obtenidas en la sección \ref{modelo cinematico}.
