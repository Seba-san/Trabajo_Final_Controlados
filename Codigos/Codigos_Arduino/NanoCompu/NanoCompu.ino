...
void serialEvent() {
  uint8_t data;//Parece que uint8_t es para byte
  if (Serial.available() > 0) {
    data= Serial.read();

    //Mando lo que recibí de la compu derecho al nano del robot
    //por RF usando Radiohead
    enviarDato(data);
  }
}


void enviarDato(uint8_t data)
{
