%% Cï¿½lculo de PID
% Este script carga los datos del ensayo al escalï¿½n para uno de los motores
% y calcula a partir de estos un controlador PID. Se estima el controlador
% usando el mï¿½todo de Ziegler Nichols. Para estudiar el controlador
% diseï¿½ado se hace una estimaciï¿½n de la planta y se verifica el efecto del
...
N=length(PWMA);%176;%Corto la seï¿½al para sacar valores ruidosos
entrada=PWMA(1:N);
salida=wA(1:N);
tiempo=tiempo(1:N);
[~,t0]=max(entrada);%Busco el valor donde la entrada pega el salto
sal0=mean(salida(1:t0-1));%Tomo el valor de correcciï¿½n como el promedio de 
%los valores de la salida cuando la entrada vale el mï¿½nimo.
ent0=min(entrada);
...
%% Estimaciï¿½n de la Planta (sï¿½lo para verificaciï¿½n)
% Acomodo las muestras para que estï¿½n equiespaciadas;
Fs=2e3;Ts=1/Fs;
...
%Tania: El primer valor al que la entrada es mï¿½xima es cuando el escalï¿½n
%empieza. Acï¿½ la idea es buscar ese valor de tiempo en el nuevo vector para
%despuï¿½s generar el vector de la seï¿½al de entrada.
...
% title('Interpolacion Lineal de w-salida');legend('Muestras Originales','Interpolaciï¿½n');
% subplot(212);stem(tiempo,entrada);hold on; plot(t,pwm_interp,'.');
% title('Interpolacion Lineal de pwm-entrada');legend('Muestras Originales','Interpolaciï¿½n');

% Estimaciï¿½n de la Planta:
...
sys2=d2d(sys,Ts2);
% Acomodo la seï¿½al de entrada a la nueva frec de muestreo:
t2=linspace(min(t),max(t),(max(t)-min(t))/Ts2);
[~,indice]=max(pwm_interp);%Busco el punto donde la seï¿½al cambia
%Busco el ï¿½ndice del valor de t2 mï¿½s prï¿½ximo al valor de tiempo donde se da
%el mï¿½nimo de pwm_interp:
[~,indice]=min(abs(t2-t(indice)));
%Armo a mano el vector de entrada para que sea un escalï¿½n perfecto con
...
% legend('Versiï¿½n Contï¿½nua','Versiï¿½n discreta 1','Versiï¿½n discreta 2');
...
%% Desempeï¿½o del controlador para la planta estimada
...
%Calculo las respuestas al escalï¿½n de los sistemas realimentados:
