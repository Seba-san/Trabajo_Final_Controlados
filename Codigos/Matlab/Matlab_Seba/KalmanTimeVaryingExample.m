...
% from the Control System Toolbox library to estimate the position and
% velocity of a ground vehicle based on noisy position measurements such as
% GPS sensor measurements. The plant model in Kalman filter has
...
% represent a disrete approximation to this kinematic relationship:
...
% rate of 1Hz. The variance of the measurment noise $v$, the R matrix, is
...
% data for a generic vehicle. Note that the diagonal Q implies a naive
% approach that assumes that the velocity changes in north and east
% direction are uncorrelated.

%% Kalman Filter Block Inputs and Setup
% The 'Kalman Filter' block is in the |Control System Toolbox| library in
% Simulink.  It is also in |System Identification Toolbox/Estimators|
% library. Configure the block parameters for discrete-time state
...
%
% * *H*: |0|. The process noise does not impact the measurments y entering
...
% time-invariant Kalman filter. Time-invariant Kalman filter performs
...
% car goes through sudden decelearation and a sharp turn, respectively.
